...
// bending base url { baseURL:'http://localhost:5000'}
const REACT_APP_API={ baseURL:process.env.REACT_APP_baseURL};
    const getTotalCount = async() => {
        try {
          const response = await axios.get(`${REACT_APP_API.baseURL}/invoices/count?searchQuery=${user?.result?._id}`);
  ...
  const handleRates =(e) => {
    ...
    <div>
        ...
        <Table className={classes.table} aria-label="simple table">
            <TableHead>
            <TableRow>
                <TableCell>Cource Details</TableCell>
                {/* <TableCell >Qty</TableCell> */}
                <TableCell>Fess</TableCell>
            ...
            {invoiceData.items.map((itemField, index) => (
                ...
                <TableCell align="right"> <InputBase sx={{ ml: 1, flex: 1 }} type="number" name="amount" onChange={e => handleChange(index, e)}  value={(1 * itemField.unitPrice) - (1 * itemField.unitPrice) * itemField.discount / 100} disabled /> </TableCell>
