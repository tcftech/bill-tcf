...
const InvoiceDetails = () => {
  ...
  const REACT_APP_API={ baseURL:process.env.REACT_APP_baseURL};
...
// base url 
    ...
    axios.post(`${REACT_APP_API.baseURL}/create-pdf`, 
    { name: invoice.client.name,
      ...
      .then(() => axios.get(`${REACT_APP_API.baseURL}/fetch-pdf`, { responseType: 'blob' }))
      .then((res) => {
  ...
  const sendPdf = (e) => {
    ...
    // react app api pdf
    axios.post(`${REACT_APP_API.baseURL}/send-pdf`, 
    { name: invoice.client.name,
      address: invoice.client.address,
      phone: invoice.client.phone,
      email: invoice.client.email,
  ...
  return (
    ...
    return (
        <div className={styles.PageLayout}>
           {invoice?.creator?.includes(user?.result?.role=="admin"?"66ddbd635a9b22475848dd4d":user?.result?._id || user?.result?.googleId) && (
            <div className={styles.buttons}>
               ...
               disabled={user.result.role?false:true}
                > 
        ...
        <TableBody>
          {invoiceData?.items?.map((itemField, index) => (
            <TableRow key={index}>
              ...
              <TableCell align="right"> <InputBase sx={{ ml: 1, flex: 1 }} type="number" name="amount"  value={(1 * itemField.unitPrice) - (1 * itemField.unitPrice) * itemField.discount / 100} readOnly /> </TableCell>
